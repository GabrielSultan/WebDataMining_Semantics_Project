...
\end{itemize}
No trafilatura is needed because text is extracted directly from the JSON response.

\subsection{Cleaning Pipeline}

The extraction pipeline: First, flatten text from nested API payloads; Second, filter URL literals (not useful for NLP); Then, normalize whitespace; Finally, apply a useful-page threshold ($\geq$500 words). Results are stored in JSONL format (\texttt{url}, \texttt{text}, \texttt{title}, \texttt{word\_count}).
