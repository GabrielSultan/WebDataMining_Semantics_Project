...
Domain: \textbf{Local History} (cultural heritage objects), focused on \textbf{French and Parisian history in English}. Per Phase~1 instructions (10 seed URLs), we use the Europeana Search API with \textbf{English} search strings as equivalent ``seeds'' (e.g.\ \texttt{Paris France history}, \texttt{French history Paris monuments}, \texttt{Palace of Versailles France history}). Each request adds the refinement \texttt{LANGUAGE:en} so retrieved records are English-language. Items with $\geq$500 words are kept.
...
We use APIs when anti-robot checking occurs. The crawler respects web ethics:
\begin{itemize}
...
The extraction pipeline: First, flatten text from nested API payloads; Second, filter URL literals (not useful for NLP); Then, normalize whitespace; Finally, apply a useful-page threshold ($\geq$500 words). Results are stored in JSONL format in the data folder (\texttt{url}, \texttt{text}, \texttt{title}, \texttt{word\_count}). Implementation in \texttt{src/crawl/phase1\_crawler.py}:
