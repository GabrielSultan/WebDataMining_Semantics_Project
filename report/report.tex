...
The statistics report (see \texttt{livrable/statistics\_report.txt}) contains the number of triplets, entities, and relations. The pipeline respects InstructionPhase2 Step~4 requirements (50k--200k triplets, 5k--30k entities, 50--200 relations):

\begin{itemize}
\item \textbf{Triplets:} 110,729 (target 50,000--200,000 met) via SPARQL 1-hop, 2-hop, and predicate-controlled expansion on Wikidata plus Europeana metadata.
\item \textbf{Entities:} 7,972 (target 5,000--30,000 met) from Wikidata URIs and Europeana entity extraction.
\item \textbf{Relations:} 50 (target 50--200 met) via a whitelist of Wikidata properties (\texttt{ALLOWED\_WIKIDATA\_PROPERTIES}) and consolidated Europeana predicates (dc:, dcterms:, edm:, one canonical predicate per semantic role).
\end{itemize}
...
\textbf{Quantitative measure.} In the final KB (\texttt{kb\_expanded.nt}), triplets whose subject is a Europeana URI (\texttt{https://www.europeana.eu/item/...}) account for the majority of the total volume (110,729 triplets). The remainder comes from the initial KB (Phase~1) and SPARQL expansion on Wikidata.
