...
The following artefacts are produced by Phase~3 and complement the Phase~2 deliverables:

\begin{itemize}
\item \texttt{livrable/kge\_splits/train.txt}, \texttt{valid.txt}, \texttt{test.txt} -- Cleaned triple splits for KGE training
\item \texttt{livrable/kge\_splits/transe\_result/}, \texttt{complex\_result/} -- Trained TransE and ComplEx models and evaluation results
\end{itemize}
...
\item \textbf{PyKEEN.} Ali et al., ``PyKEEN: A Python Library for Learning and Evaluating Knowledge Graph Embeddings,'' JMLR 22, 2021. \url{https://github.com/pykeen/pykeen}
\item \textbf{OWLReady2.} Lamy, ``OWLReady2: A Python package for ontology-oriented programming,'' J. Biomedical Semantics 8, 2017.
\item \textbf{TransE.} Bordes et al., ``Translating Embeddings for Modeling Multi-relational Data,'' NeurIPS 2013.
\item \textbf{ComplEx.} Trouillon et al., ``Complex Embeddings for Simple Link Prediction,'' ICML 2016.
\end{itemize}
...
We have implemented a complete pipeline from unstructured web data (Europeana API) to a structured Knowledge Base aligned with Wikidata/DBpedia, and extended it with knowledge reasoning in Phase~3. The pipeline uses transformer-based NER for entity recognition, dependency parsing for relation extraction, and SPARQL-based expansion (1-hop, 2-hop, predicate-controlled) to reach the target volume and relation diversity. Phase~3 adds rule-based reasoning with SWRL (OWLReady2, Pellet) and embedding-based reasoning with TransE and ComplEx, achieving a link prediction MRR of 0.53 with ComplEx on the expanded KB.
