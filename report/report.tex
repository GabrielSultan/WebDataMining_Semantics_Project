  ...
  \textbf{Machine.} The experiments run on a standard development machine with Python~3.9+ and sufficient RAM to load the expanded KB (approximately 109k triples).

  \textbf{Model.} We use Ollama with \texttt{gemma:2b} (or alternatively \texttt{qwen3.5:0.8b}, \texttt{llama3.2:1b}). The model is invoked via the REST API at \texttt{http://localhost:11434/api/generate}.

  \textbf{Graph source.} The RDF graph is \texttt{livrable/kb\_expanded.nt} (N-Triples format), produced by the Phase~2 pipeline. It contains entities and relations from Europeana, Wikidata, and the initial Local History KB.

  \subsection{Method}

  \textbf{Schema summary.} We build a compact schema summary for prompting: (1)~prefixes from the graph plus defaults (rdf, rdfs, dc, dcterms, edm, lh, wdt); (2)~distinct predicates (up to 80); (3)~distinct classes from \texttt{rdf:type} (up to 40); (4)~sample triples (up to 20). This summary is injected into the LLM prompt so it uses only known IRIs.

  \textbf{SPARQL generation.} The LLM receives the schema summary and the user question. It is instructed to return a valid SPARQL~1.1 SELECT query in a fenced code block. We extract the query with a regex and execute it via rdflib.

  \textbf{Self-repair.} If the generated SPARQL fails (syntax error, unknown predicate, etc.), we send the error message back to the LLM with the original question and bad query, asking for a corrected SPARQL. We execute the repaired query; if it still fails, we report the error.

  \subsection{Evaluation}

  We evaluate on at least five questions. For each, we record the baseline answer (LLM without KG), the SPARQL-gen RAG result (rows returned), and whether the result is correct.

  \begin{table}[ht]
  \centering
  \caption{Evaluation: baseline vs.\ SPARQL-generation RAG.}
  \begin{tabular}{p{4.5cm}p{3cm}p{3.5cm}l}
  \toprule
  \textbf{Question} & \textbf{Baseline (No RAG)} & \textbf{SPARQL-gen RAG} & \textbf{Correct?} \\
  \midrule
  What places are associated with Bucarest? & Hallucination / generic & Rows from dcterms:spatial & Yes \\
  How many items have subject archaeology? & Approximate / wrong & COUNT result from dc:subject & Yes \\
  What entity types exist? & Generic list & Person, Place, Org from rdf:type & Yes \\
  Which items are linked to Romania? & Vague / incomplete & Europeana URIs + lh: entities & Yes \\
  What creators appear in the base? & Invented names & dc:creator / edm:agent values & Yes \\
  \bottomrule
  \end{tabular}
  \end{table}

  \textbf{Screenshots.} See the running script with 1--2 representative examples in the supplementary material. The CLI shows: baseline answer, then SPARQL query used, repaired flag, and result rows.

  \subsection{Discussion}

  \textbf{Accuracy.} SPARQL-generation RAG yields grounded results from the KB, whereas the baseline often hallucinates or gives generic answers for domain-specific questions (e.g., ``What places are associated with Bucarest?''). The baseline has no access to the graph and cannot answer precisely.

  \textbf{Failure cases.} The LLM may generate invalid SPARQL (typos, wrong prefixes, non-existent predicates). Self-repair helps when the error message is informative (e.g., ``Unknown predicate''); it is less effective when the query is semantically wrong but syntactically valid (e.g., wrong variable binding).

  \textbf{Self-repair.} In practice, self-repair corrects common mistakes such as missing PREFIX declarations or incorrect IRI usage. A single repair attempt is often sufficient; repeated failures suggest the question may be too complex or the schema summary insufficient.

  \textbf{Scalability.} For large KGs (millions of triples), loading the full graph into memory may be impractical. A production system would use a SPARQL endpoint (e.g., Apache Jena Fuseki, Virtuoso) and query it remotely. The schema summary size (80 predicates, 40 classes, 20 samples) remains bounded and does not scale with graph size; the main bottleneck is KB load time and LLM latency.
